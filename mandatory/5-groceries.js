...
//Get all array at the same level with FLAT method
let collectAllPlanValues = Object.values(weeklyMealPlan).flat();
//Finding duplicated values
let toFindDuplicates = (values) =>
  values.filter((item, index) => values.indexOf(item) === index);
  //Executing the toFindDuplicates function
let weeklyGroceriesToBuy = toFindDuplicates(collectAllPlanValues);
...
for (let days of Object.keys(weeklyMealPlan)) {
    //console.log(weeklyMealPlan[days].length); // Will display contents of the object inside the array
    for ( let days of Object.keys(numberOfItemsPerWeek)){
